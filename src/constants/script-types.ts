...
export const SCRIPT_TYPES: ScriptTypeConfig[] = [
  ...
  },
  {
    ...
    format: '输出3-5个独立方案，每个方案用三级标题分隔，格式如下：\n\n### 方案一：[钩子类型名称]\n\n每个方案包含2-3个分镜，每个分镜用时间段标题分隔，字段逐行输出：\n\n#### 0-2s 钩子\n- **景别**：特写/近景/中景/全景\n- **画面**：1句话简述视觉冲击内容\n- **镜头**：运镜方式\n- **音效**：音频标注\n\n#### 2-4s 爆点\n- **景别**：特写/近景/中景/全景\n- **画面**：1句话简述爆点内容\n- **字幕**：屏幕文字（如有）\n- **镜头**：运镜方式\n- **音效**：音频标注\n\n#### 4-5s CTA\n- **画面**：行动号召画面\n- **字幕**：CTA文案\n- **音效**：音频标注',
    rules: ['每个方案必须用 ### 方案X：[钩子类型] 作为标题，禁止用其他分隔方式', '每个方案独立成篇，互不关联', '第1秒必须有视觉/听觉冲击', '5秒内必须制造"想看下去"的悬念', '分镜字段逐行输出，禁止合并到一行'],
  },
  {
  ...
  },
  {
    ...
    format: '输出分为两部分：\n\n**第一部分：玩法策划简案**\n- **核心循环**：完整的"操作→反馈→动机"闭环\n- **上瘾机制**：靠什么让用户想继续玩\n- **30秒体验设计**：用户依次经历哪些玩法节点及情绪曲线\n- **关键爽点**：2-3个触发下载冲动的玩法瞬间\n- **差异化设计**：与同类素材的核心区别\n\n**第二部分：分镜脚本**\n每个分镜用以下固定格式输出，Shot标题独占一行，字段用 "- 字段名: 内容" 格式：\n\nShot 1 | 0-3s | 入门操作\n- 景别: 特写\n- 画面: 1-2句简述（服务于玩法展示）\n- 玩家操作: 具体手势/交互动作\n- 游戏反馈: 操作触发的即时视觉/音效反馈\n- 镜头: 运镜方式\n- 字幕: 屏幕文字\n- 音效: 音频标注\n- 转场: 转场方式\n\nShot 2 | 3-8s | 正反馈爆发\n（格式同上，依此类推）',
    rules: ['必须先输出完整的玩法策划简案再写分镜', '每个分镜必须对应明确的玩法节点，禁止纯氛围/纯视觉镜头', '画面描述服务于玩法展示，简洁明确，不写大段视觉描写', '前3秒必须让观众看懂"这个游戏在玩什么"', 'Shot标题必须独占一行，格式严格为：Shot N | 时间段 | 玩法节点', '关键爽点时刻用特写/慢镜头放大操作反馈'],
  },
]
