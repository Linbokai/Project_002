...
export function buildDirectionDetailPrompt(
  ...
  const dirLabel = ['一', '二', '三'][directionNumber - 1]
  return `我选择了**方向${dirLabel}**，请基于这个方向生成完整的买量视频分镜脚本。

## 当前配置
- ${configCtx}

## 要求
1. 严格按照方向${dirLabel}的核心策略、钩子类型和叙事结构来创作
2. 参考补充上下文中的可复用元素和优化建议
3. **必须严格按照 system prompt 中「脚本类型」定义的输出格式，逐字段、逐镜头输出分镜脚本**
4. 每个镜头必须包含独立的字段行：景别、画面、台词/旁白、镜头运动、转场、音效等
5. 禁止输出策略概要或方向分析，只输出可直接执行的分镜脚本内容
6. 分镜数量和时间分配必须覆盖完整的 ${config.duration} 秒视频时长`
}
