...
const displayTemplates = computed(() => {
  const byUsage = templateStore.getTemplatesByUsage()
  if (byUsage.length > 0 && templateStore.getUsageCount(byUsage[0]!.id) > 0) {
      ...
      <div class="flex w-full flex-col gap-2.5">
        <div class="flex items-center justify-between">
          <span class="text-xs font-medium text-muted-foreground">
            {{ displayTemplates.some((t) => templateStore.getUsageCount(t.id) > 0) ? '最近使用' : '推荐模板' }}
          </span>
          <button
            type="button"
